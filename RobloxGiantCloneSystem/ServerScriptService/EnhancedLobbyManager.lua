-- Enhanced Lobby Manager Script
-- Place this in ServerScriptService
-- This version integrates with the giant system
...
-- Function to notify giant system
local function notifyGiantSystem(lobbyCode)
	local lobby = lobbies[lobbyCode]
	if not lobby then return end
	
	-- Call global function if it exists
	if _G.UpdateLobbyGiants then
		_G.UpdateLobbyGiants(lobbyCode, lobby.players)
	end
	
	-- Also fire bindable event if it exists
	local lobbyGiantSync = ReplicatedStorage:FindFirstChild("LobbyGiantSync")
	if lobbyGiantSync then
		lobbyGiantSync:Fire(lobbyCode, lobby.players)
	end
end
...
local function createLobby(owner)
	...
	-- Notify giant system
	task.spawn(function()
		wait(0.5) -- Give time for player to receive lobby info
		notifyGiantSystem(code)
	end)
...
lobbyEvents.OnServerEvent:Connect(function(player, action, data)
	...
	elseif action == "JoinLobby" then
		local code = data.code:upper() -- Make case-insensitive
...
task.spawn(function()
	while true do
		...
		for code, lobby in pairs(lobbies) do
			-- Check if all players are still in game
			local validPlayers = {}
			...
			if #validPlayers == 0 then
				lobbies[code] = nil
			elseif #validPlayers < #lobby.players then
...
print("EnhancedLobbyManager loaded successfully")
