...
export async function resolveProvider(): Promise<Provider> {
  ...
  if (!hasSupabase) {
    throw new Error(
      "Kredensial Supabase tidak diset. Set NEXT_PUBLIC_SUPABASE_URL dan NEXT_PUBLIC_SUPABASE_ANON_KEY (atau SUPABASE_URL/SUPABASE_ANON_KEY)."
    )
  }
...
export async function getServices() {
  // Provider is always supabase now
  ...
  return { ...supabaseServices, productService }
}
